/*
SOLUTION: dp[i][j][k] = max score if Bessie is at round i, has moved j times already,
and is doing move k (0 = hoof, 1 = paper, 2 = scissors)
*/
#include <bits/stdc++.h>
...
int main() {
	...
	int ans = 0;
	F0R(i,k+1) F0R(j,3) ans = max(ans,dp[n-1][i][j]);
	cout << ans;
}	
